...
!!! info "🚧 研发中的可选项"
    **Pad** 与**数采背包**目前**尚在研发中**,属可选配置,现阶段可能不随货提供;实际以合同配置为准。
...
- 备齐:左/右主夹爪、两根 **USB 锁紧线**、笔记本/数采终端。线缆**夹爪端固定为 Type-C 锁紧接头**,另一端按数采终端的接口选 **Type-C 或 Type-A** 均可。
...
![双臂接好后的 lsusb 输出](assets/hardware/lsusb.jpg){ width="720" }

上图是**双臂**接好后的实际输出,对照着看三类设备:

| 输出里的字样 | 是什么 | 双臂应有 |
|---|---|---|
| `Xense Robotics ... GSPS01…` | 视触觉传感器 | **4 个** |
| `Sunplus ... XCA…` | 腕部鱼眼相机 | **2 个** |
| `QinHeng Electronics USB Dual_Serial` | 夹爪 MCU 串口 | **2 个** |

序列号末位还能直接看出左右(单左双右,见 [序列号与左右识别](#sn))——图中 `…0069`/`…0071` 是左,
`…0070`/`…0072` 是右。其余条目是主机自身的 hub、网卡、蓝牙等,与夹爪无关。
...
!!! warning "没标定的主夹爪连不上,必须先标"
    ...
    并提示要跑哪条命令——双臂两台都要标 → [4.1 夹爪标定](04-calibration.md#41)
