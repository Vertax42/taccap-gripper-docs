...
上图是**双臂**接好后的实际输出。**两个红框各对应一只主夹爪**,每框内是这只爪的 3 个 UVC 设备:
...
所以**双臂共 6 个 UVC**、单臂 3 个。框外紧邻的 `QinHeng Electronics USB Dual_Serial`
是夹爪 MCU 串口,**每只爪 1 个**;其余条目是主机自身的 hub、网卡、蓝牙等,与夹爪无关。
