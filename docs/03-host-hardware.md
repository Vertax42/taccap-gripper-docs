...
!!! danger "[3.1](#31) 与 [3.2](#32) 是必做项,不做就连不上夹爪"
    这两项都是**主机侧的一次性配置**,做完长期有效:

    - **[3.1 串口权限](#31)** —— 用户不在 `dialout` 组,夹爪能被列出但**打不开串口**,
      `firmware_sn` 为空、连接失败。
    - **[3.2 屏蔽 ModemManager](#32)** —— 不做的话每次热插拔后的几秒内串口被占,
      表现为**时好时坏的 `Device or resource busy`**,最容易被误判成线缆或带宽问题。

    两项都跳过的直接后果就是**连不上夹爪**;后面的标定、预览、录制都无从谈起。
...
### 已配置好的设备:只需做三件事 {#pico-preconfigured}

出厂预配置的头显已经完成了开发者模式、电源策略、APP 安装、追踪器绑定和追踪模式设置——
这些都写在头显里,断电不丢。你**每次采集前**仍要做的只有:

1. **追踪器开机** —— **短按**电源键,蓝灯亮起即可。
2. **[网络连接](#pico-network)** —— 接 USB、在 APP 里填数采电脑的 IP。**每次接线都要做**,
   而且数采期间要关掉电脑的 WiFi。
3. **[界面清单](#pico-toolkit-ui) 四项核对**,然后 **[启动对齐](#pico-frame)** —— `Send` 必须
   最后勾;世界系原点在 APP 启动那一刻冻结,**整份数据集期间不要重启 APP**。
...
!!! tip "开机是短按,配对才要长按"
    **日常开机**只需**短按**电源键,蓝灯亮起即可。
    **只有首次绑定**才长按约 6 秒到**蓝红交替闪烁**——平时别按到进配对状态。
