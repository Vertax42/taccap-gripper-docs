...
!!! warning "开录前先确认 `gripper.pos` 张到底能到 1.0"
    数据集里的 `gripper.pos` 是归一化开度(`0.0` 闭合 / `1.0` 张开),这两个端点来自写在
    MCU flash 里的**编码器零点**和**行程上限**。预览时把夹爪张到机械极限看读数:
    **顶不到 1.0**(例如停在 `0.68`)就说明这台没标行程上限,去标一次。

    标定值存在 flash 里,断电不丢、换主机也不用重标——所以这是**发现刻度不对时才做**的事,
    不是每次开录的例行步骤。
