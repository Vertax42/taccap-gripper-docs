...
### 「V2.1」指的是命令集版本,不是固件构建号 {#v21}

夹爪固件有**两套编号**,文档里都会出现:

- **命令集版本**(`V1.9` / `V2.0` / `V2.1`)—— 固件支持哪些命令。每条命令按引入时的版本
  标注,比如行程标定用的 `EncoderMaxCal` 是 **V2.1** 引入的,所以标定要求 ≥ V2.1。
- **固件构建号**(`1.2.0` / `1.1.0`)—— 具体那一版镜像。

对应关系是固定的:**带命令集 V2.1 的构建就是 leader `1.2.0` / follower `1.1.0`**。所以刷写
命令里写 `--target-version 1.2.0.0`,和「升到 V2.1」说的是同一件事。主从两个号不同,是因为
它们本来就是两份不同的固件。
...
`TCGU01A28Z0023m` → `m` → 主夹爪,用 `tc-gu-01-master.bin`。同一套双臂设备上两只夹爪常常**都是主夹爪**。
